...
static void gatts_cb(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        //add caracteristic (read/write/notify)
        {
            ...
            err = esp_ble_gatts_add_char(gl_service_handle, &char_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE | ESP_GATT_CHAR_PROP_BIT_NOTIFY, &char_val, NULL);
    ...
    case ESP_GATTS_ADD_CHAR_EVT:
        ...
        //add cccd descriptor for notify
        {
            ...
            err = esp_ble_gatts_add_char_descr(gl_service_handle, &descr_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, &descr_val, NULL);
            if (err != ESP_OK) {
    ...
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
        ...
        if (led_char_added && !led_cccd_added) {
            ...
            printf("add char DHT -> %s",esp_err_to_name(err));
        } else if (gl_dht_value_handle != 0 && gl_dht_cccd_handle ==0) {
            gl_dht_cccd_handle = param->add_char_descr.attr_handle;
            printf("mapped dht_cccd_handle = %d\n", gl_dht_cccd_handle);
        } else {
            printf("descriptor mapping ambigous / ignored\n");
