...
static bmp280_conf_t bmp_conf;
static int32_t raw_temp_val, raw_press_val;

void i2c_init(bmp280_conf_t *conf) {
...
static esp_err_t i2c_write_reg(uint8_t reg, uint8_t value) {
    ...
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(bmp_conf.i2c_port, cmd, pdMS_TO_TICKS(1000));
...
void bmp280_update_raw(void) {
	bmp280_read_raw(&raw_press_val, &raw_temp_val);
}

double bmp280_get_temp(void) { 
	return bmp280_compensate_temp(raw_temp_val) / 100.0; 
}
double bmp280_get_pressure(void) {
	return bmp280_compensate_press(raw_press_val);

}
double bmp280_get_altitude(void) {
    return 44330.0 * (1.0 - pow(bmp280_get_pressure() / 101325.0, 0.190295));
}
